...
func _test_weapon_table(t: Node) -> void:
	...
	t.eq(pistol.damage, 11.0, "手枪基础伤害 11")
	...
	t.eq(shotgun.energy_cost, 9.0, "霰弹枪耗能 9")
...
func _test_projectile_hit(t: Node) -> void:
	...
	t.gte(player.last_shot_damage, 9.9, "手枪伤害不低于 11*(1-10%)")
	t.lte(player.last_shot_damage, 21.8, "手枪伤害不超过 11*(1+10%)*1.8（含暴击上限）")
...
func _test_shotgun_pellets(t: Node) -> void:
	...
	t.near(player.energy(), 91.0, 0.6, "霰弹枪扣除 9 点能量")
	...
	t.gte(dummy.total_damage, 14.0, "多枚弹丸累计伤害（单枚基础 8）")
...
func _test_laser_hitscan(t: Node) -> void:
	...
	t.gte(dummy.total_damage, 17.0, "激光伤害不低于 19*(1-10%)")
	t.near(player.energy(), 86.0, 0.6, "激光枪扣除 14 点能量")
...
func _test_skill_area_damage(t: Node) -> void:
	...
	# 把靶子挪到玩家身侧（技能半径 92），等一帧让物理空间同步新位置
	dummy.reset(200.0)
	...
	t.near(player.energy(), 75.0, 0.6, "技能扣除 25 点能量")
	t.near(player.skill_cooldown_timer, 6.0, 0.05, "技能冷却 6 秒")
	t.gte(dummy.hit_count, 1.0, "冲击波命中范围内的靶子")
	t.gte(dummy.total_damage, 17.0, "冲击波伤害不低于 34*50%（边缘衰减下限）")
